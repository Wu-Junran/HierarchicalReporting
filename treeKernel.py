...
bio_dataset = ['MUTAG', 'PTC']
social_dataset = ['IMDBBINARY', 'IMDBMULTI', 'COLLAB']
...
def gridSearch(dataset, tree_depth):
    ...
    target_acc = 0
    target_std = 0
    gamma = 'auto' if dataset in ['IMDBBINARY', 'IMDBMULTI'] else 'scale'
    for tagV in range(4):
        if dataset in social_dataset and tagV in [1, 3]:
            continue
        print(dataset, tree_depth, 'tag%s' % tagV, gamma, flush=True)
        ...
        for r in range(10):
            ...
            if acc_mean <= max_acc:
                break
            max_acc = acc_mean
            if max_acc > target_acc:
                target_acc, target_std, target_tag = acc_mean, np.array(accs).std(), tagV
            print(r,
                  '%.6f' % max_c,
                  '%.6f' % acc_mean,
                  '[%s]' % ', '.join(['%.4f' % a for a in accs]),
                  flush=True)
    ...
    print(dataset, tree_depth, target_tag,
          '%.4f' % target_acc*100,
          '%.4f' % target_std*100, flush=True)
